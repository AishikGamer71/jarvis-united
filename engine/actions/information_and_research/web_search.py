...
def web_search(
    ...
    # --- SEARCH MODE: try Gemini grounded search first, fallback to DDG ---
    try:
        result = _gemini_search(query)
        print("[WebSearch] Gemini grounded search OK.")
        return result
    except Exception as e:
        print(f"[WebSearch] Gemini search failed ({e}) - falling back to DuckDuckGo...")

    try:
        results = _ddg_search(query)
        result  = _format_ddg(query, results)
        print(f"[WebSearch] DuckDuckGo: {len(results)} result(s).")
        return result
    except Exception as e:
