...
export default function registerDropZoneControl(ipcMain: IpcMain) {
  ipcMain.handle(
    "ghost-drag-and-drop",
    async (_event, { startX, startY, endX, endY }) => {
      try {
        const primaryDisplay = screen.getPrimaryDisplay();
        const scaleFactor = primaryDisplay.scaleFactor;

        const start = new Point(
          Math.round(startX / scaleFactor),
          Math.round(startY / scaleFactor),
        );
        const end = new Point(
          Math.round(endX / scaleFactor),
          Math.round(endY / scaleFactor),
        );
  ...
  ipcMain.handle(
    ...
    async (_event, { sourcePath, targetFolder }) => {
      try {
