...
export default function registerAdbHandlers(ipcMain: IpcMain) {
  ...
  const saveDeviceToHistory = async (
    ip: string,
    port: string,
    model: string,
  ) => {
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          await saveDeviceToHistory(
            ip,
            port,
            modelOut.trim().toUpperCase() || "UNKNOWN DEVICE",
          );
        } catch (e) {}
  ...
  ipcMain.handle("adb-telemetry", async () => {
    ...
    try {
      ...
      const isCharging =
        batteryOut.includes("AC powered: true") ||
        batteryOut.includes("USB powered: true");

      const level = levelMatch ? parseInt(levelMatch[1]) : 0;
      const temp = tempMatch ? (parseInt(tempMatch[1]) / 10).toFixed(1) : 0;

      const { stdout: storageOut } = await execAsync(
        `adb ${target} shell df -h /data`,
      );
  ...
  ipcMain.handle("get-mobile-info-ai", async () => {
    if (!activeDevice)
      return "Error: You are not currently connected to any mobile device.";
    try {
      const target = `-s ${activeDevice.ip}:${activeDevice.port}`;
      const { stdout: batOut } = await execAsync(
        `adb ${target} shell dumpsys battery`,
      );
  ...
  ipcMain.handle("adb-close-app", async (_, { packageName }) => {
    ...
    try {
      ...
      if (packageName === "android.media.action.STILL_IMAGE_CAMERA") {
        await execAsync(
          `adb ${target} shell am force-stop com.google.android.GoogleCamera`,
        );
        return { success: true };
      }
  ...
  ipcMain.handle("adb-get-notifications", async () => {
    ...
    try {
      const { stdout } = await execAsync(
        `adb ${target} shell dumpsys notification --noredact`,
      );
  ...
  ipcMain.handle(
    "adb-push-file",
    async (_, { sourcePath, destPath = "/sdcard/Download/" }) => {
      if (!activeDevice)
  ...
  ipcMain.handle("adb-hardware-toggle", async (_, { setting, state }) => {
    ...
    try {
      ...
      if (cleanSetting === "bluetooth" || cleanSetting === "bt") {
        try {
          await execAsync(`adb ${target} shell svc bluetooth ${action}`, {
            timeout: 5000,
          });
        } catch (e) {
          await execAsync(
            `adb ${target} shell cmd bluetooth_manager ${action}`,
            { timeout: 5000 },
          );
        }
        return { success: true };
      ...
      if (cleanSetting === "wifi") {
        try {
          await execAsync(`adb ${target} shell svc wifi ${action}`, {
            timeout: 5000,
          });
        } catch (e) {
          const wifiState = state ? "enabled" : "disabled";
          await execAsync(
            `adb ${target} shell cmd wifi set-wifi-enabled ${wifiState}`,
            {
              timeout: 5000,
            },
          );
      ...
      if (cleanSetting === "data" || cleanSetting === "mobile data") {
        await execAsync(`adb ${target} shell svc data ${action}`, {
          timeout: 5000,
        });
        return { success: true };
      }
      ...
      if (cleanSetting === "airplane" || cleanSetting === "flight") {
        await execAsync(
          `adb ${target} shell cmd connectivity airplane-mode ${action}`,
          {
            timeout: 5000,
          },
        );
      ...
      if (cleanSetting === "location" || cleanSetting === "gps") {
        ...
        await execAsync(
          `adb ${target} shell settings put secure location_mode ${locState}`,
          {
            timeout: 5000,
          },
        );
