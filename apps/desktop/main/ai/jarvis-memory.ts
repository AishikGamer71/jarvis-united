...
export default function registerIpcHandlers({
  ...
  const OLD_FILE_PATH_APP = path.join(
    app.getPath("userData"),
    "Chat",
    "jarvis_memory.json",
  );
  const OLD_IRIS_PATH_APP = path.join(
    app.getPath("userData"),
    "Chat",
    "iris_memory.json",
  );
  ...
  if (!fs.existsSync(FILE_PATH)) {
    if (fs.existsSync(OLD_FILE_PATH_APP)) {
      try {
        fs.copyFileSync(OLD_FILE_PATH_APP, FILE_PATH);
      } catch (e) {}
    } else if (fs.existsSync(OLD_IRIS_PATH_APP)) {
      try {
        fs.copyFileSync(OLD_IRIS_PATH_APP, FILE_PATH);
      } catch (e) {}
    }
  }
