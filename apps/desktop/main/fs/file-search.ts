...
export default function registerFileSearch(ipcMain: IpcMain) {
  ipcMain.handle("index-folder", async (event, folderPath: string) => {
    try {
      ...
      const dbPath = path.join(app.getPath("userData"), "jarvis_semantic_db");
      ...
      async function scanForIndexing(dir: string) {
        ...
        for (const entry of entries) {
          ...
          if (entry.isDirectory()) {
            ...
            if (VALID_INDEX_EXTENSIONS.has(path.extname(nameLower)))
  ...
  ipcMain.handle("search-files", async (event, { query, groqKey }) => {
    try {
      ...
      if (!groqKey || groqKey.trim() === "") {
        throw new Error(
          "Missing Groq API Key. Please configure it in the Command Center Vault.",
        );
      }
      ...
      const runSemantic = async () => {
        try {
          ...
          const results = await table
            .search(Array.from(queryOutput.data))
            .limit(3)
            .execute();
      ...
      const runNativeCrawler = async () => {
        ...
        try {
          ...
          if (Array.isArray(parsed.keywords))
            searchParams.keywords = parsed.keywords;
        ...
        if (rawInput) {
          if (
            os.platform() === "win32" &&
            (rawInput.length === 1 || rawInput.includes("drive"))
          ) {
