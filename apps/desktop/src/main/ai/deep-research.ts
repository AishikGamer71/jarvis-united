...
export default function registerDeepResearch({
  ipcMain,
}: {
  ipcMain: IpcMain;
}) {
  ipcMain.handle(
    ...
    async (event, { query, tavilyKey, groqKey }) => {
      try {
        if (!tavilyKey || !groqKey) {
          throw new Error(
            "Missing API Keys. Please configure Tavily and Groq in the Command Center.",
          );
        }
