...
const getStateDir = () =>
  path.join(app.getPath("userData"), "jarvis_scan_states");
...
const saveState = async (state: ScanState) => {
  try {
    ...
    await fs.writeFile(
      getStateFilePath(state.dirPath),
...
export default function registerOracle({ ipcMain }: { ipcMain: IpcMain }) {
  ...
  ipcMain.handle("ingest-codebase", async (event, { dirPath, geminiKey }) => {
    try {
      if (!geminiKey) {
        throw new Error(
          "Missing Gemini API Key. Please configure it in the Command Center Vault.",
        );
      }
  ...
  ipcMain.handle(
    "consult-oracle",
    async (_event, { query, geminiKey, groqKey }) => {
      try {
        ...
        if (!geminiKey || !groqKey) {
          throw new Error(
            "Missing API Keys. Ensure both Gemini and Groq are configured in Settings.",
          );
        }
