            ...
            <div className="flex justify-center gap-6 py-4">
              {folders.map((folder) => {
                const isTargeted =
                  ...
                  (folder.id === "images" &&
                    targetCategory?.includes("photo")) ||
                  (folder.id === "documents" &&
                    targetCategory?.includes("pdf")) ||
                  (folder.id === "code" && targetCategory?.includes("dev")) ||
                  (folder.id === "videos" &&
                    (targetCategory?.includes("video") ||
                      targetCategory?.includes("mp4"))) ||
                ...
                return (
                  <motion.div
                    key={folder.id}
                    animate={
                      isTargeted ? { scale: 1.05, y: -5 } : { scale: 1, y: 0 }
                    }
