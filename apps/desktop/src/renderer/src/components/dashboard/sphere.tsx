...
const CustomParticleSphere = ({ count = 1000 }) => {
  ...
  // Track speaking state globally to use in the render loop without re-renders
  const stateRef = useRef(false);
  useEffect(() => {
    const handleState = (e: any) => {
      stateRef.current = e.detail === "SPEAKING";
    };
  ...
  useFrame((state, delta) => {
    ...
    const targetVolume = stateRef.current ? 0.6 + Math.random() * 0.3 : 0;
    currentVolume.current += (targetVolume - currentVolume.current) * 0.15;
    ...
    const currentPos = mesh.current.geometry.attributes.position
