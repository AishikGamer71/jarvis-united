export const openMobileApp = async (packageName: string) => {
  try {
    const res = await window.electron.ipcRenderer.invoke("adb-open-app", {
      packageName,
    });
    if (res.success) {
...
export const closeMobileApp = async (packageName: string) => {
  try {
    const res = await window.electron.ipcRenderer.invoke("adb-close-app", {
      packageName,
    });
    if (res.success) {
...
export const tapMobileScreen = async (xPercent: number, yPercent: number) => {
  try {
    const res = await window.electron.ipcRenderer.invoke("adb-tap", {
      xPercent,
      yPercent,
    });
    if (res.success) {
...
export const fetchMobileInfo = async () => {
  try {
    const result =
      await window.electron.ipcRenderer.invoke("get-mobile-info-ai");
    return result;
  } catch (e) {
...
export const pushFileToMobile = async (
  sourcePath: string,
  destPath?: string,
) => {
  try {
    const res = await window.electron.ipcRenderer.invoke("adb-push-file", {
      sourcePath,
      destPath,
    });
    if (res.success) {
...
export const pullFileFromMobile = async (
  sourcePath: string,
  destPath?: string,
) => {
  try {
    const res = await window.electron.ipcRenderer.invoke("adb-pull-file", {
      sourcePath,
      destPath,
    });
    if (res.success) {
