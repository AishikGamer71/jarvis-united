export const ingestCodebase = async (dirPath: string): Promise<string> => {
  try {
    const geminiKey = localStorage.getItem("jarvis_custom_api_key") || "";
    ...
    window.dispatchEvent(
      new CustomEvent("oracle-ingest-start", { detail: { path: dirPath } }),
    );
    ...
    const cleanupListener = window.electron.ipcRenderer.on(
      ...
      (_ipcEvent: any, data: any) => {
        ...
        window.dispatchEvent(
          new CustomEvent("oracle-progress", { detail: payload }),
        );
      },
    );
    ...
    if (result.success) {
      window.dispatchEvent(
        new CustomEvent("oracle-ingest-done", {
          detail: { chunks: result.totalChunks },
        }),
      );
...
export const consultOracle = async (query: string): Promise<string> => {
  try {
    const geminiKey = localStorage.getItem("jarvis_custom_api_key") || "";
    const groqKey = localStorage.getItem("jarvis_groq_api_key") || "";
    ...
    if (result.success) {
      window.dispatchEvent(
        new CustomEvent("oracle-answered", {
