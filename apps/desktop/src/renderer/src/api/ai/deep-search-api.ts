export const runDeepResearch = async (query: string): Promise<string> => {
  try {
    window.dispatchEvent(
      new CustomEvent("deep-research-start", { detail: { query } }),
    );

    const tavilyKey = localStorage.getItem("jarvis_tailvy_api_key") || "";
    ...
    window.dispatchEvent(
      new CustomEvent("deep-research-done", { detail: { success: false } }),
    );
