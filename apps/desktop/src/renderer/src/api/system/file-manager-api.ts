...
export const writeFile = async (fileName: string, content: string) => {
  try {
    return await window.electron.ipcRenderer.invoke("write-file", {
      fileName,
      content,
    });
  } catch (err) {
...
export const manageFile = async (
  ...
  try {
    return await window.electron.ipcRenderer.invoke("file-ops", {
      operation,
      sourcePath,
      destPath,
    });
...
export const createFolder = async (path: string) => {
  try {
    return (await window.electron.ipcRenderer.invoke("create-directory", path))
      .success
      ? `✅ Created: ${path}`
      : "❌ Failed.";
  } catch (e) {
