export const readGalleryImages = async () => {
  try {
    const images: any[] =
      await window.electron.ipcRenderer.invoke("get-gallery");
    if (!images || images.length === 0)
      return "Visual Vault is empty. No images found.";
...
export const analyzeDirectPhoto = async (
  filePath: string,
      ...
      reader.onloadend = () => {
        ...
        if (socket && socket.readyState === WebSocket.OPEN) {
          socket.send(
            JSON.stringify({
              realtimeInput: {
                mediaChunks: [{ mimeType: "image/png", data: base64data }],
              },
            }),
          );
          resolve(
