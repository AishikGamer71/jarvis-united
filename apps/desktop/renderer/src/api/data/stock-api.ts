...
export const compareStocks = async (ticker1: string, ticker2: string) => {
  try {
    const [res1, res2] = await Promise.all([
      fetchYahooData(ticker1),
      fetchYahooData(ticker2),
    ]);
    ...
    for (let i = 0; i < t1.length; i++) {
      if (c1[i]) {
        ...
        chartMap.set(timeStr, {
          time: timeStr,
          price1: parseFloat(c1[i].toFixed(2)),
        });
      }
    }
    ...
    for (let i = 0; i < t2.length; i++) {
      if (c2[i]) {
        ...
        if (chartMap.has(timeStr)) {
          ...
          chartMap.set(timeStr, {
            time: timeStr,
            price2: parseFloat(c2[i].toFixed(2)),
          });
        }
      }
    }
    ...
    const finalData = {
      ...
      percentChange1: ((change1 / res1.meta.chartPreviousClose) * 100).toFixed(
        2,
      ),
      percentChange2: ((change2 / res2.meta.chartPreviousClose) * 100).toFixed(
        2,
      ),
