...
import { cancelIngestion } from "@renderer/api/ai/rag-oracle-api";
  ...
  useEffect(() => {
    ...
    const onProgress = (e: any) => {
      ...
      if (e.detail.status === "scanning") {
        ...
        if (e.detail.file !== "Scan Complete")
          addLog(`Found: ${e.detail.file}`);
    ...
    const onIngestDone = (e: any) => {
      if (status !== "cancelled") {
        ...
        setMessage(
          e.detail.message ||
            `Successfully processed ${e.detail.chunks} vectors.`,
        );
        setTimeout(() => setStatus("idle"), 5000);
      }
    };
        ...
        >
          ...
          <div className="h-16 bg-emerald-500/10 border-b border-emerald-500/20 flex items-center justify-between px-8">
            <div className="flex items-center gap-4">
              ...
              {status === "cancelled" && (
                <Octagon className="w-6 h-6 text-red-500" />
              )}
              <span
              ...
              <motion.div layout className="flex flex-col gap-8">
                  ...
                  <div className="w-full h-3 bg-zinc-900 rounded-full overflow-hidden border border-zinc-800 relative">
                    ...
                    <motion.div
                      ...
                      animate={{
                        width:
                          targetTotalFiles > 0
                            ? `${(fileCount / targetTotalFiles) * 100}%`
                            : "0%",
                      }}
                ...
                <div className="grid grid-cols-4 gap-4">
                  ...
                  <StatBox
                    icon={Clock}
                    label="Time Taken"
                    textValue={formatTime(elapsedMs)}
                  />
                  <StatBox
                    icon={Timer}
                    label="Time Left"
  ...
  >
    ...
    {textValue ? (
      ...
      <div className="flex items-baseline gap-1">
        ...
        {total !== undefined && (
          <span className="text-lg font-mono text-zinc-500">/ {total}</span>
        )}
      </div>
    )}
  </div>
