...
export class GeminiLiveService {
  ...
  async connect(): Promise<void> {
    ...
    if (window.electron?.ipcRenderer) {
      ...
      this.apiKey = secureKeys?.geminiKey || localStorage?.getItem('jarvis_custom_api_key') || '';
    } else {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.socket.onopen = () => {
        ...
        this.socket?.send(JSON.stringify({ type: 'api_key', value: this.apiKey }));
        ...
        this.socket?.send(JSON.stringify({ type: 'set_temperature', value: temperature }));
        this.socket?.send(JSON.stringify({ type: 'set_max_tokens', value: maxTokens }));
      ...
      this.socket.onmessage = (event) => {
        try {
          ...
          if (data.type === 'state') {
            window.dispatchEvent(new CustomEvent('jarvis-state', { detail: data.value }));
            ...
            if (typeof data.value === 'string') {
              if (data.value.startsWith('Jarvis: ')) {
                ...
                if (msgText) {
                  import('./jarvis-ai-brain').then(({ saveMessage }) => saveMessage('jarvis', msgText));
                }
              } else if (data.value.startsWith('You: ')) {
                const msgText = data.value.substring(5).trim();
                if (msgText) {
                  import('./jarvis-ai-brain').then(({ saveMessage }) => saveMessage('user', msgText));
                }
              }
            }
          } else if (data.type === 'terminal_execute') {
            window.dispatchEvent(new CustomEvent('jarvis-terminal-execute', { detail: data.value }));
          }
        } catch (err) {}
      };
